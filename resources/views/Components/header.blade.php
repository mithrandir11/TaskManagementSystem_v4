<nav class="border-b border-gray-200  py-2.5 ">
    <div class="flex flex-wrap justify-between items-center mx-auto ">
        ...
        <div class="flex items-center lg:order-2">
            @auth
            <a href="{{route('dashboard')}}" class="hover:text-gray-800  duration-200  hover:bg-gray-50 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2  border">{{auth()->user()->name}}</a>
            @else
            <a href="{{route('login')}}" class="hover:text-gray-800  duration-200  hover:bg-gray-50 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2  border">ورود به حساب</a>
            @endauth
